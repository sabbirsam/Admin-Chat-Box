...
class ACB_Enqueue extends ACB_BaseController{
    ...
    public function ACB_admin_enqueue($screen){
        ...
        if("toplevel_page_admin_chat_box" == $screen ){
            // wp_enqueue_style( 'acb_fontawesome_min_css', $this->plugin_url .'assets/library/font-awesome.min.css');
            wp_enqueue_script( 'acb_bootstrap_min_js', $this->plugin_url .'assets/library/bootstrap.min.js',array('jquery'),1.0,true );
            wp_enqueue_script( 'acb_seeetalert_min_js', $this->plugin_url .'assets/library/sweetalert2@11.js',array('jquery'),1.0,true );

            /**
             * Chat box setting save 
             */
            wp_enqueue_script( 'acb_chat_js', $this->plugin_url . 'assets/admin/js/settings.js',array('jquery'),1.0,true ); 
            wp_localize_script( 'acb_chat_js', 'acb_data_truncate', array(
                'ajaxurl'=>admin_url("admin-ajax.php", null)
                ) );
                wp_enqueue_script('jquery');
                wp_enqueue_script('acb_chat_js');
